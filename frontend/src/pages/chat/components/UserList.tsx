  ...
  return (
    <div className='h-full border-r border-zinc-800 overflow-hidden'>
      <div className='flex flex-col h-full items-start py-5 overflow-y-auto'>
        {users?.map((user) => (
          <div key={user.clerkId} onClick={() => setSelectedUser(user)} className={`grid grid-cols-[auto_2fr] items-center justify-start space-x-2 ${user.clerkId === selectedUser?.clerkId ? 'bg-zinc-950' : ''} hover:bg-zinc-950 w-full p-2 cursor-pointer`}>
