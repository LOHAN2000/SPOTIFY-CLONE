...
export const SongsTable = () => {
  ...
  return (
    <div className='flex flex-col p-0 sm:p-3 space-y-3 h-full overflow-auto'>
      ...
      {songs.map((song) => (
        <div key={song.song_id} className='grid grid-cols-[3fr_2fr_2fr_1fr] sm:grid-cols-[3fr_1fr_1fr_1fr] items-center justify-center hover:bg-zinc-600/20'>
          ...
          <div className='flex items-center justify-center text-red-400/80'><Trash onClick={() => deleteSong(song.song_id)} className='size-4 sm:size-6 cursor-pointer'/></div>
        </div>
      ))}
    </div>
  )
